...
def _try_chromestats_fallback(extension_id: str):
    """
    Fallback: download extension via alternate source when primary download fails.
...
def extension_downloader_node(state: WorkflowState) -> Command:
    ...
    Uses a fallback download source when the primary download fails (e.g. on cloud servers).
